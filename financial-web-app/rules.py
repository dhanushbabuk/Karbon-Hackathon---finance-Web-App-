...
def total_revenue(data: dict, financial_index):
    ...
    """
    try:
        # Accessing the financial data at the specified index
        financial_data = data.get("financials", [])[financial_index]
        
        # Extracting the net revenue from the Profit and Loss statement
        revenue = financial_data.get("pnl", {}).get("lineItems", {}).get("netRevenue", 0.0)
...
def total_borrowing(data: dict, financial_index: int) -> float:
    ...
    line_items = balance_sheet.get("lineItems", {})

    long_term_borrowing = line_items.get("longTermBorrowings", 0.0)
    short_term_borrowing = line_items.get("shortTermBorrowings", 0.0)
    ...
    borrowing_to_revenue_ratio = total_borrowings / revenue
...
def iscr(data: dict, financial_index: int) -> float:
    ...
    pbit = line_items.get("profitBeforeInterestAndTax", 0.0)
    depreciation = line_items.get("depreciation", 0.0)
    interest_expense = line_items.get("interestExpense", 0.0)
